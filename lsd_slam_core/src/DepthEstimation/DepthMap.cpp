...
bool DepthMap::observeDepthCreate(const int &x, const int &y, const int &idx, RunningStats* const &stats) {
  ...
  if (refFrame->getTrackingParent() == activeKeyFrame) {
    ...
    if (wasGoodDuringTracking != nullptr && !wasGoodDuringTracking[(x >> SE3TRACKING_MIN_LEVEL) + (width >> SE3TRACKING_MIN_LEVEL)*(y >> SE3TRACKING_MIN_LEVEL)]) {
      if (plotStereoImages)
  ...
  for (int y = 0; y < height; y++)
    for (int x = 0; x < width; x++) {
      ...
      if (!(u_new > 2.1f && v_new > 2.1f &&
            u_new < width-3.1f && v_new < height-3.1f)) {
        if (enablePrintDebugInfo)
      ...
      if (!targetBest->isValid) {
        ...
        *targetBest = DepthMapPixelHypothesis(
            new_idepth, new_var, source->validity_counter);
  ...
  for (int y = yMin; y < yMax; y++) {
    for (int x = 3; x < width-2; x++) {
      ...
      if ((dest->blacklisted >= MIN_BLACKLIST &&
        ...
        // find a good value for the filled depth
...
void DepthMap::updateKeyframe(std::deque<std::shared_ptr<Frame> > referenceFrames) {
  ...
  for (std::shared_ptr<Frame> frame : referenceFrames) {
    ...
    // [question] zli: why not just push_back?
...
void DepthMap::createKeyFrame(Frame* new_keyframe) {
  ...
  // [note] zli: change activeKeyFrame
...
inline float DepthMap::doLineStereo(
  ...
  if (useSubpixelStereo) {
    ...
    // - if zero-crossing occurs exactly in between (gradient Inconsistent),
      ...
      if (enablePrintDebugInfo && (gradPost_this*gradPost_this > 0.1f*0.1f*gradPost_post*gradPost_post || gradPre_this*gradPre_this > 0.1f*0.1f*gradPre_pre*gradPre_pre))
