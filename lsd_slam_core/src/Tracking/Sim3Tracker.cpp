...
Sim3 Sim3Tracker::trackFrameSim3(
  ...
  for (int lvl = startLevel; lvl >= finalLevel; lvl--) {
    ...
    if (plotSim3TrackingIterationInfo)
      callOptimized(calcSim3Buffers, (reference, frame, referenceToFrame, lvl, true));
    ...
    for (int iteration = 0; iteration < settings.maxItsPerLvl[lvl]; iteration++) {
      ...
      while (true) {
        ...
        if (plotSim3TrackingIterationInfo)
          callOptimized(calcSim3Buffers, (reference, frame, new_referenceToFrame, lvl, true));
        numCalcResidualCalls[lvl]++;
...
#if defined(ENABLE_NEON)
Sim3ResidualStruct Sim3Tracker::calcSim3WeightsAndResidualNEON(const Sim3& referenceToFrame) {
